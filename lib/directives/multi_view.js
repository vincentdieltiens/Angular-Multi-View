	...
	directive('vdMultiView', ['$http', '$templateCache', '$route', '$anchorScroll', '$compile',
		...
		return {
			...
			link: function(scope, element, attr) {
				...
				element.bind('$destroyLastScope', function(e) {
					e.stopPropagation();

					if (!$(this).has(e.target).length) {
						element.find('[vd-multi-view]:first, [data-vd-multi-view]:first').trigger('$destroyLastScope', {});
					}
				...
				function destroyLastScope() {	
					if (lastScope) {
						element.trigger('$destroyLastScope', {});
						lastScope.$destroy();
						lastScope = null;
					}
				}
